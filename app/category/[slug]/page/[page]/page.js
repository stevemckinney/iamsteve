...
export default async function BlogCategory({ params, searchParams }) {
  ...
  const posts = await Promise.all(
    allPosts
      .filter((post) => post.categories.includes(data.slug.charAt(0).toUpperCase() + data.slug.slice(1)))
  ...
  return (
    <div className="grid col-container grid-cols-subgrid gap-y-12 pb-32 pt-18 frame frame-outset-top">
      ...
      <div className="col-content">
        <Pagination total={pagination.total} current={pagination.current} category={params.slug} />
      </div>
    </div>
  )
}
