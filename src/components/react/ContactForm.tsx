...
export default function ContactForm() {
  ...
  useEffect(() => {
    ...
    const recentSubmissions = submissions.filter(
      (time: number) => now - time < RATE_LIMIT_DURATION
    )
  ...
  if (isRateLimited || FORM_DISABLED) {
    return (
      <div className="flex w-full p-12 shadow-(--shadow-notice) col-prose gap-3 leading-tight bg-(--color-notice-bg) rounded-md justify-center rounded-sm">
