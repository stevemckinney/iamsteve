...
if (isset($config))
{
	...
	$images_path                       = $base_path . DIRECTORY_SEPARATOR . $images_folder;
	$images_url                        = $base_url . DIRECTORY_SEPARATOR . $images_folder;

	$env_config['index_page']          = '';
	$env_config['site_index']          = '';
	$env_config['base_url']            = $base_url . DIRECTORY_SEPARATOR;
	...
	$env_config['upload_preferences'] = array(
  	1 => array(
  		'name' => 'Downloads',
  		'server_path' => $base_path . '/uploads/downloads/',
  		'url' => $base_url . '/uploads/downloads/'
  	),
  	2 => array(
  		'name' => 'Portfolio',
  		'server_path' => $base_path . '/uploads/portfolio/',
  		'url' => $base_url . '/uploads/portfolio/'
  	),
  	3 => array(
  		'name' => 'Blog',
  		'server_path' => $base_path . '/uploads/blog/',
  		'url' => $base_url . '/uploads/blog/'
  	)
  );
	 ...
	 */
	$env_config['disable_all_tracking']        = 'n'; // If set to 'y' some of the below settings are disregarded
	...
	$env_config['enable_hit_tracking']         = 'y';
	$env_config['enable_entry_view_tracking']  = 'y';
  ...
  $env_config['stash_file_sync'] = FALSE; // set to TRUE to sync stash embed file changes during development
  $env_config['stash_static_basepath'] = $base_path . '/system/user/templates/default_site/static_cache/';
  ...
  $env_config['stash_static_cache_enabled'] = FALSE; // set to TRUE to enable static caching
	 ...
	 */
	$env_config['new_version_check']        = 'y'; // no slowing my CP homepage down with this
