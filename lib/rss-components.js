...
export function Blockquote({ children, style }) {
  ...
  const config = styles[style] || { prefix: '', bgColor: '#f9fafb', borderColor: '#d1d5db' }
 ...
 */
export function BentoGridShell({ link = 'https://stevemckinney.github.io/bento-grid/option-1.html' }) {
  return `<div style="
    margin: 2em 0;
    padding: 1.5em;
    background: #f0f9ff;
...
export function Image({ src, alt = '', width, height }) {
  ...
  return `<img src="${src}" alt="${alt}" ${attrs.join(' ')} style="max-width: 100%; height: auto;" />`
}
...
export function Images({ children, description, compare }) {
  ...
  return `<figure style="margin: 2em 0;">${children}${description ? `<figcaption style="margin-top: 0.5em; font-style: italic; color: #666;">${description}</figcaption>` : ''}${compareNote}</figure>`
}
