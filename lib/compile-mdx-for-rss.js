  ...
  return content.replace(
    ...
    (match, body) => {
      ...
      for (const line of lines) {
        ...
        } else if (/^(?:AI|ai):\s*/.test(line)) {
          align = 'ai'
          text = line.replace(/^(?:AI|ai):\s*/, '')
        }
      ...
      const messages = grouped
        .map(({ align, lines: msgLines }) => {
          const inner =
            msgLines.length === 1
              ? msgLines[0]
              : msgLines.map((t) => `<p>${t}</p>`).join('')
          return `<ChatMessage align="${align}">${inner}</ChatMessage>`
        })
        .join('')
...
export function compileMdxForRssWithMarked(mdxContent) {
  ...
  // Transform components with children (inner-first to handle nesting)
  // Repeat until no more replacements occur, so nested components
  // like ChatMessage inside Chat are resolved before their parent.
  const withChildrenRegex = new RegExp(
    `<(${componentPattern})(\\s+[^>]*?)?>([^]*?)<\\/\\1>`,
    'gs'
  )
