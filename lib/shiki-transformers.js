...
/**
 * Wraps bare CSS custom property declarations in a dummy :root {} block so
 * Shiki's CSS grammar enters property-list context and highlights values
 * correctly. The wrapper tokens are stripped from the output.
 *
 * Detects code blocks where every non-empty, non-comment line is a custom
 * property declaration (starts with --) or a continuation (indented/nested).
 */
export function transformerCSSPartialProperties() {
  return {
    name: 'css-partial-properties',
    preprocess(code) {
      ...
      // Check if every non-empty line looks like a bare declaration block
      // (starts with --, or is indented continuation, or is a comment, or is just punctuation like closing parens)
      const isBareProperties = nonEmpty.every((l) => {
        const t = l.trim()
        return (
          t.startsWith('--') ||
          t.startsWith('/*') ||
      ...
      if (!isBareProperties || nonEmpty.length === 0) return code

      this._wrappedPartial = true
      return `:root {\n${code}\n}`
    },
    tokens(tokens) {
      if (!this._wrappedPartial) return tokens

      // Remove the first line (:root {) and last line (})
