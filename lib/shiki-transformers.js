 ...
 * @theme for :root before tokenisation (which the grammar does understand),
 * then patches the tokens back to display @theme.
 */
    ...
    preprocess(code) {
      ...
      for (let i = 0; i < lines.length; i++) {
        if (/^\s*@theme\s*\{/.test(lines[i])) {
          ...
          lines[i] = lines[i].replace('@theme', ':root ')
        }
      }
    ...
    tokens(tokens) {
      ...
      for (const lineIdx of this._themeLines) {
        ...
        for (const token of line) {
          if (token.content === ':root' || token.content === ':root ') {
            token.content = '@theme'
            break
          }
        }
