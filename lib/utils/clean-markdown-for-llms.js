  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (inFigure) {
      ...
      if (line.includes('<figcaption>')) {
        ...
        const captionMatch = captionLine.match(/<figcaption>(.*?)<\/figcaption>/s)
        if (captionMatch) {
    ...
    line = line.replace(/<Blockquote\s+style="notice">(.*?)<\/Blockquote>/g, '> **Note:** $1')

    // <LinkFigma href="url">text</LinkFigma> → [text](url)
    line = line.replace(/<LinkFigma\s+href="([^"]+)">(.*?)<\/LinkFigma>/g, '[$2]($1)')

    // <LinkGithub href="url">text</LinkGithub> → [text](url)
    line = line.replace(/<LinkGithub\s+href="([^"]+)">(.*?)<\/LinkGithub>/g, '[$2]($1)')

    // <Demo src="url" /> → [View interactive demo](url)
    line = line.replace(/<Demo\s+src="([^"]+)"\s*\/>/g, '[View interactive demo]($1)')
