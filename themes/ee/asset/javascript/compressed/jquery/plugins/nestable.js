...
var n=this.options.constrainToRoot?this.el:e.body;t(n).append(this.dragEl),this.options.constrainToRoot?
// Account for root element offset if appending to root element
this.dragEl.css({left:s.pageX-this.el.offset().left-a.offsetX,top:s.pageY-this.el.offset().top-a.offsetY}):this.dragEl.css({left:s.pageX-a.offsetX,top:s.pageY-a.offsetY});
// total depth of dragging item
var d,h,r=this.dragEl.find(this.options.itemNodeName+"."+this.options.itemClass);for(d=0;d<r.length;d++)h=t(r[d]).parents(this.options.listNodeName+"."+this.options.listClass).length,h>this.dragDepth&&(this.dragDepth=h)},dragStop:function(t){var s=this.dragEl.children(this.options.itemNodeName+"."+this.options.itemClass).first();s[0].parentNode.removeChild(s[0]),this.placeEl.replaceWith(s),this.dragEl.remove(),this.el.trigger("change"),this.hasNewRoot&&this.dragRootEl.trigger("change"),this.reset()},dragMove:function(i){var a,o,n,d,h,r=this.options,p=this.mouse;this.options.constrainToRoot?
// Account for root element offset if appending to root element
this.dragEl.css({left:i.pageX-this.el.offset().left-p.offsetX,top:i.pageY-this.el.offset().top-p.offsetY}):this.dragEl.css({left:i.pageX-p.offsetX,top:i.pageY-p.offsetY}),
...
var g=this.pointEl.closest("."+r.rootClass),m=this.dragRootEl.data("nestable-id")!==g.data("nestable-id");/**
             * move vertical
             */
if(!p.dirAx||m||f){
