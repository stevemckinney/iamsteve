...
updateEditor:function(){this.$editor.html(i.Formatting.getBrowserMarkupFrom(this.$field)),this.selectEmptyParagraph()},/**
	 ...
	 */
selectEmptyParagraph:function(){var n,i=this.$editor,r=(i.html(),window.getSelection());this.isEmpty()&&(i.html(this._empty()),n=e.createRange(),r.removeAllRanges(),n.selectNodeContents(i.find("p").get(0)),
...
t.browser.mozilla&&i.find("p").eq(0).html(""),r.addRange(n))}},i.Editor.constructor=i.Editor,
// ---------------------------------------------------------------------
/**
 * Element Manager
 *
 ...
 */
// ---------------------------------------------------------------------
i.Event=function(e){this.Editor=e,this.$editor=e.$editor,this.eventHandlers=[],this.textStart=null,this.pasteStart=null,this.textDeleting=!1,// typing backwards ;)
// helper classes
this.Undo=e.Undo,this.Selection=e.Selection,this._hijack_events(),
// special events
this.add("paste",i.Paster.getHandler(e))},i.Event.prototype={/**
	 * Add a handler for editor events. These
	 * are things such as "bold" or "paste". Not
	 * browser events!
	 */
add:function(e,t){this.eventHandlers[e]=t},/**
	 * Do we have a handler?
	 */
has:function(e){return e in this.eventHandlers},/**
	 ...
	 */
run:function(e,t,n){var i=this.eventHandlers[e](t,n);
...
i!==!1&&n()},/**
	 ...
	 * $editor.fire('bold')
	 */
fire:function(e){var n,i,r=this;
// special case - undo and redo
if(this._saveTextState(e),"undo"==e||"redo"==e){var o,s="undo"==e?"hasUndo":"hasRedo";return void(this.Undo[s]()&&(o=this.Undo[e](this.$editor.html()),this.$editor.html(o[0]),this.Selection.set(o[1])))}
// mark text change
...
return!this.has(e)||(n=this.getState(),i=function(){this.hasRun||(this.hasRun=!0,r.textChange(n),r._saveTextState(e),r.$editor.focus())},void this.run(e,n,t.proxy(i,i)))},/**
	 ...
	 * @return void
	 */
textChange:function(e,t){t=t||this.getState(),
// this.Editor.updateField();
this.Editor.selectEmptyParagraph(),this.Undo.push(e.html,t.html,e.selection,t.selection),this.$editor.closest("form").trigger("entry:startAutosave")},/**
...
return!t.altGraphKey&&(t.metaKey&&(n+="cmd-"),t.altKey&&(n+="alt-"),t.ctrlKey&&(n+="ctrl-"),t.shiftKey&&(n+="shift-"),!o&&e.length>1?n.replace(/-$/,"")==e:(i=r[t.keyCode],!!i&&e.toLowerCase()==(n+i).toLowerCase()))},/**
	 ...
	 * isEvent('copy', evt)
	 */
isEvent:function(e,t){var n=t.type;
...
getState:function(){return{html:this.$editor.html(),selection:this.Selection.get()}},/**
	 ...
	 * typed so far.
	 */
_saveTextState:function(e){
// some events shouldn't affect this
// @todo figure out the whole list
"redo"!=e&&this.textStart&&(this.textChange(this.textStart),this.textStart=null)},
// ---------------------
...
"selectionchange focusin mousedown":t.proxy(this._rangeEvent,this),"keydown keyup keypress":t.proxy(this._keyEvent,this),"cut undo redo paste input contextmenu":t.proxy(this._menuEvent,this),focus:t.proxy(this._focusEvent,this)};this.$editor.on(e)},_focusEvent:function(){this.Editor.isEmpty()&&
// on an empty editor we want to completely replace
// otherwise the first paragraph gets munged
this.Editor.selectEmptyParagraph()},/**
	 ...
	 * correct event.
	 */
_keyComboEvent:function(e){var t,n=["undo","redo","paste"];if("keydown"==e.type)for(;t=n.shift();)if(this.isEvent(t,e))return"paste"==t?(this.fire(t),!0):(e.preventDefault(),this.fire(t),!1);return!0},/**
	 * Handle key events
	 *
...
if("keypress"==e.type)return!0;if(e.ctrlKey||e.altKey||e.metaKey)return this._keyComboEvent(e);if("keydown"==e.type)"backspace"==r[e.keyCode]?0==this.textDeleting&&(this.textDeleting=!0,this._saveTextState("backspace")):1==this.textDeleting&&(this.textDeleting=!1,this._saveTextState("keypress")),null==this.textStart&&(this.textStart=this.getState());else if("keyup"==e.type)switch(r[e.keyCode]){case"up":case"down":case"left":case"right":this._saveTextState("keyup")}},/**
	 ...
	 */
_rangeEvent:function(e){this._saveTextState(e.type)},/**
	 ...
	 */
_menuEvent:function(e){for(var t,n=["undo","redo","paste"];t=n.shift();)this.isEvent(t,e)&&("paste"!=t&&e.preventDefault(),this.fire(t))}},i.Event.constructor=i.Event,
// ---------------------------------------------------------------------
/**
 * Undo Class
 *
...
i.Undo=function(){this.max_depth=75,this.saved=[],this.index=0},i.Undo.prototype={/**
	 ...
	 */
push:function(e,n,i,r){var o=[],s=this;o=t.isArray(e)?t.map(e,function(e,t){return s._diff(e,n[t])}):this._diff(e,n),o&&(
...
undo:function(e){this.index--;for(var t=this.saved[this.index],n=t.changes,i=n.length,r=0;r<i;r++)change=n[r],e=e.substring(0,change[0])+change[1]+e.substring(change[0]+change[2].length);return[e,t.selection[0]]},/**
	 ...
	 */
redo:function(e){for(var t=this.saved[this.index],n=t.changes,i=n.length,r=i-1;r>=0;r--)change=n[r],e=e.substring(0,change[0])+change[2]+e.substring(change[0]+change[1].length);return this.index++,[e,t.selection[1]]},/**
	 * Undo available?
	 */
...
hasRedo:function(){return this.index!=this.saved.length},/**
	 ...
	 * are identical.
	 */
_diff:function(e,t){var n,i=e.length,r=t.length,o=0,s=0;
// easiest case
if(e==t)return null;
 ...
 */
// ---------------------------------------------------------------------
i.Selection=function(e){this.$editor=e,this.top=this.$editor.get(0)},i.Selection.prototype={_replace:new RegExp("[\r\n]","g"),/**
	 * Get current selection offsets based on
	 * the editors *text* (not html!).
	 *
	 * @return [startIndex, endIndex]
	 */
get:function(t){var i,r,o=window.getSelection(),s=e.createRange();if(t===n){if(!o.rangeCount)return[0,0];t=o.getRangeAt(0)}return i=t.toString().replace(this._replace,"").length,s.setStart(this.top,0),s.setEnd(t.startContainer,t.startOffset),r=s.toString().replace(this._replace,"").length,[r,r+i]},/**
	 ...
	 */
set:function(i,r){t.isArray(i)&&(r=i[1],i=i[0]);var o,s,a=window.getSelection(),l=e.createRange();o=this._getOffsetNode(this.top,i,!0),l.setStart.apply(l,o),
// collapsed
r===n||r==i?(r=i,l.collapse(!0)):(s=this._getOffsetNode(this.top,r,!1),l.setEnd.apply(l,s)),a.removeAllRanges(),a.addRange(l)},/**
	 ...
	 */
toString:function(e){var t=window.getSelection();return e===n&&(e=t.getRangeAt(0)),e.toString()},/**
...
t.inArray(s.nodeName.toLowerCase(),d)>-1&&null!==s.nextSibling&&(s=s.nextSibling);c&&s.firstChild&&"br"!=s.firstChild.nodeName.toLowerCase();)c--,s=s.firstChild}}return a=s.nodeValue?s.nodeValue.length:0,[s,a+n]}},i.Selection.constructor=i.Selection,
// ---------------------------------------------------------------------
/**
 * Editor Commands
 *
...
t.each(i.makeEasy,function(e,t){n.make[t]=function(e){n.execCommand(t,!1,e)}}),
// Fill in the simple is() commands
t.each(i.isSelectors,function(t,r){t in i.isNativeState?n.is[t]=function(){return n.selectionIsWithin(r)||e.queryCommandState(i.isNativeState[t])}:n.is[t]=function(){return n.selectionIsWithin(r)}});
...
return t.each(r.is,function(e,t){n.is[e]=function(){return n.is[t]()}}),t.each(r.make,function(e,i){n.make[e]=t.proxy(n.make,i)}),n.noSpans=function(){try{return e.execCommand("styleWithCSS",0,!1),function(){e.execCommand("styleWithCSS",0,!1)}}catch(n){try{return e.execCommand("useCSS",0,!0),function(){e.execCommand("useCSS",0,!0)}}catch(n){try{return e.execCommand("styleWithCSS",!1,!1),function(){e.execCommand("styleWithCSS",!1,!1)}}catch(n){return t.noop}}}}(),n}(),/**
 * Add the more complex commands
 */
t.extend(i.Commands,{_blockElements:i.Element.getContentElements().join(",").replace(",div,",",div:not(."+i.name+"-editor),"),
...
getRanges:function(){for(var e,t=window.getSelection(),n=t.rangeCount,i=[];n--;)e=t.getRangeAt(n),i.push(e);return i},/**
	 ...
	 */
restoreRanges:function(e){var t=window.getSelection(),n=e.length;for(t.removeAllRanges();n--;)t.addRange(e[n])},/**
	 * Changes the parent html block element
	 * into the one passed in by the dev.
	 * Ex. Use to flip headings.
	 */
changeContentBlock:function(e){for(var n,i=window.getSelection(),r=this,o=t(r),s="WysiHat-replaced",a=i.rangeCount,l=[];a--;)n=i.getRangeAt(a),l.push(n),this.getRangeElements(n,this._blockElements).each(function(){r.replaceElement(t(this),e)}).data(s,!0);o.children(e).removeData(s),this.restoreRanges(l)},/**
...
unformatContentBlock:function(){this.changeContentBlock("p")},/**
	 ...
	 */
unlinkSelection:function(){this.manipulateSelection(function(e){this.getRangeElements(e,"[href]").each(this.clearElement)})},/**
 ...
 * generated.
 */
t.extend(i.Commands.make,{blockquote:function(){i.Commands.is.indented()?i.Commands.unquoteSelection():i.Commands.quoteSelection()},alignment:function(e){i.Commands.execCommand("justify"+e)},backgroundColor:function(e){var n=t.browser.mozilla?"hilitecolor":"backcolor";i.Commands.execCommand(n,!1,e)}});
// ---------------------------------------------------------------------
/**
 * Commands Mixin
 *
...
this.isAncestorOf(n,s,!0)||(s=n),!s.canHaveHTML)return new r(s.parentNode,this.getNodeIndex(s));var l,d,c,h,u,f=e.createElement("span"),m=i?"StartToStart":"StartToEnd";
