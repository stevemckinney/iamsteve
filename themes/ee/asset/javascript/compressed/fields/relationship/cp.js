 ...
 * @filesource
 */
"use strict";!function(e){e(document).ready(function(){function a(a,r,i){var n=e(a).closest("fieldset").find("div.col.last").eq(0),d=e(a).closest("fieldset").serialize(),c=EE.publish.field.URL+"/"+e(n).find(".relate-wrap").data("field"),o=e(a).attr("name");if(0==n.length){n=e(a).closest("td");var h=e(n).data("row-id")?e(n).data("row-id"):0;d=e(n).find("input").serialize()+"&column_id="+e(n).data("column-id")+"&row_id="+h,c=EE.publish.field.URL+"/"+e(a).closest("table").attr("id")}r&&(d+="&channel="+r),
...
var t=e("input[name="+o+"]",n).focus(),l=t.val();t.val(""),t.val(l),e(".w-8.relate-wrap .scroll-wrap",n).sortable(s)}})},i)}
// Single Relationship:
...
// Sortable!
var s={axis:"y",cursor:"move",handle:".relate-reorder",items:"label"};e(".w-8.relate-wrap .scroll-wrap").sortable(s),e(".publish form").on("submit",function(a){e(".w-8.relate-wrap .scroll-wrap").each(function(){var a,t=e(this).closest(".relate-wrap").siblings(".relate-wrap").first();
// Adding a new grid row will enable all the disabled sort fields
e(this).find('input:hidden[name$="[sort][]"]').attr("disabled","disabled");var l=1;e(this).find("label.relate-manage").each(function(){a=t.find('input[name$="[data][]"][value='+e(this).data("entry-id")+"]").closest("label");var s=a.find('input:hidden[name$="[sort][]"]').first();s.removeAttr("disabled"),s.val(l),l++})})})})}(jQuery);