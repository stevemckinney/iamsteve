 ...
 * @filesource
 */
"use strict";!function(e){e(document).ready(function(){function a(a,s,i){var n=e(a).closest("fieldset").find("div.col.last").eq(0),d=e(a).closest("fieldset").serialize(),c=EE.publish.field.URL+"/"+e(n).find(".relate-wrap").data("field"),o=e(a).attr("name");
// Assume it's in a Grid
if(0==n.length){n=e(a).closest("td");var h=e(n).data("row-id")?e(n).data("row-id"):e(n).data("new-row-id");d=e(n).find("input").serialize()+"&column_id="+e(n).data("column-id")+"&row_id="+h,c=EE.publish.field.URL+"/"+e(a).closest("table").attr("id")}s&&(d+="&channel="+s),
...
var t=e("input[name="+o+"]",n).focus(),l=t.val();t.val(""),t.val(l),e(".w-8.relate-wrap .scroll-wrap",n).sortable(r)}})},i)}
// Single Relationship:
//   When the radio button is clicked, copy the chosen data into the
//   div.relate-wrap-chosen area
e("div.publish").on("click",".relate-wrap input:radio",function(a){var t=e(this).closest(".relate-wrap"),l=e(this).closest("label"),r=e(this).closest(".scroll-wrap").data("template").replace(/{entry-id}/g,e(this).val()).replace(/{entry-title}/g,l.data("entry-title")).replace(/{channel-title}/g,l.data("channel-title"));t.find(".relate-wrap-chosen .no-results").closest("label").hide().removeClass("block"),t.find(".relate-wrap-chosen .relate-manage").remove(),t.find(".relate-wrap-chosen").first().append(r),t.removeClass("empty")}),
// Multiple Relationships
//   When checkbox is clicked, copy the chosen data into the second
//   div.relate-wrap div.scroll-wrap area
e("div.publish").on("click",".relate-wrap input:checkbox",function(a){var t=e(this).closest(".relate-wrap").siblings(".relate-wrap").first(),l=e(this).closest("label"),r=e(this).closest(".scroll-wrap").data("template").replace(/{entry-id}/g,e(this).val()).replace(/{entry-title}/g,l.data("entry-title")).replace(/{channel-title}/g,l.data("channel-title"));
...
// Sortable!
var r={axis:"y",cursor:"move",handle:".relate-reorder",items:"label"};e(".w-8.relate-wrap .scroll-wrap").sortable(r),e(".publish form").on("submit",function(a){e(".w-8.relate-wrap .scroll-wrap").each(function(){var a,t=e(this).closest(".relate-wrap").siblings(".relate-wrap").first();
// Adding a new grid row will enable all the disabled sort fields
e(this).find('input:hidden[name$="[sort][]"]').attr("disabled","disabled");var l=1;e(this).find("label.relate-manage").each(function(){a=t.find('input[name$="[data][]"][value='+e(this).data("entry-id")+"]").closest("label");var r=a.find('input:hidden[name$="[sort][]"]').first();r.removeAttr("disabled"),r.val(l),l++})})})})}(jQuery);