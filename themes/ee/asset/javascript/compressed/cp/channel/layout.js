...
a.after($("<em></em>").append(a.data("required"))),a.parents("fieldset").addClass("invalid");else if(d.test(t)){for(var o=!1,u=0;u<EE.publish_layout.length;u++)EE.publish_layout[u].id==l&&(o=!0);if(o)
// Show the duplicate_tab_name alert
a.after($("<em></em>").append(a.data("duplicate"))),a.parents("fieldset").addClass("invalid");else{var f={fields:[],id:l,name:t,visible:!0};EE.publish_layout.push(f);var p=$("ul.tabs li").length;n.find("li a").droppable("destroy"),n.append('<li><a href="" rel="t-'+p+'">'+t+'</a> <span class="tab-remove"></span></li>'),s.filter(".t-"+(p-1)).after('<div class="tab t-'+p+'"></div>'),i(),
// Update tabs
...
$("div.publish form").on("click",".sub-arrow",function(i){var t=e(),l=a(this);EE.publish_layout[t].fields[l].collapsed=!EE.publish_layout[t].fields[l].collapsed,i.preventDefault()}),$("div.publish form").on("submit",function(e){$('input[name="field_layout"]').val(JSON.stringify(EE.publish_layout))}),
// put debug back
$("body").append(l)});